...
### 18. i18n — СДЕЛАНО
- Свой словарь, не `vscode-nls`: ключ — английский исходный текст, `t("Working…")`.
- `src/l10n.ts` (хост), `media/l10n.mjs` (вебвью), `l10n/ru.json` — 198 строк.
- Язык берётся из `ompcode.language` (`auto` | `en` | `ru`), а не из языка VS Code:
  штатный `vscode.l10n` умеет следовать только языку редактора, для русского нужен
  Language Pack и переключается весь VS Code целиком.
- Строки манифеста — исключение: `%key%` + `package.nls.json` / `package.nls.ru.json`,
  их VS Code читает до загрузки расширения, поэтому они следуют языку редактора.
- Бандл вшивается в HTML вебвью тегом `<script type="application/json">`, поэтому
  смена языка требует `reloadHtml()`, а не сообщения.
- `test/l10n.test.ts` сканирует исходники на `t("…")` и роняет сборку на непереведённой
  строке, осиротевшем переводе и несовпадении плейсхолдеров.
